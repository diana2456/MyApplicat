...
import com.example.myapplication.databinding.ItemTaskTwoBinding
...
class AdapterRealEstate @Inject constructor(private val context: Context) : ListAdapter<RealEstate, AdapterRealEstate.ViewHolder>(
    DiffCallback()
) {
    ...
    inner class ViewHolder(val binding: ItemTaskTwoBinding) : RecyclerView.ViewHolder(binding.root) {
        ...
        fun bind(item: RealEstate) {
            ...
            with(binding) {
                ivPhoto.setImageResource(item.image)
                tvTitle.text = item.tvTitle
                tvDil.text = item.tvDil
                tvRoom.text = item.tvRoom
                tvKm.text = item.tvKm
                tvSan.text = item.tvSan
                tvLocation.text = item.tvLocation
            }
    ...
    @SuppressLint("Range")
    private fun readCursorData(coffeeItem: RealEstate, viewHolder: ItemTaskTwoBinding, position: Int) {
